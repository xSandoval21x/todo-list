...
let selectedProjectId =
  JSON.parse(localStorage.getItem(LOCAL_STORAGE_SELECTED_PROJECT_ID)) || 1;
...
deleteListButton.addEventListener('click', () => {
  ...
  const importantTodos = selectedProject.todos.filter(
    (item) => item.important == true
  );
  projects = projects.filter((project) => project.id !== selectedProjectId);
  // remove important todos from the important project list
  importantProject.todos = importantProject.todos.filter(
    (item) => !importantTodos.includes(item)
  );
  selectedProjectId = 1;
  render();
  save();
...
submitTodoButton.addEventListener('click', () => {
  ...
  // require name
  if (todoTitleInput.value == null || todoTitleInput.value === '') return;
...
function renderProjectItems() {
  ...
  selectedProject.todos.forEach((todo) => {
    ...
    const completeItemButton = itemElement.querySelector(
      '[data-complete-item]'
    );
    const importantItemButton = itemElement.querySelector(
      '[data-important-item]'
    );
    ...
    itemDate.innerText =
      todo.date === '' ? '' : format(parseISO(todo.date), 'PPP');
